...
Keep the homepage focused on reader decisions: explain who it serves, show a few examples with concrete uses and input needs, offer a copyable first-generation prompt, and retain the precise pack directory generated by `scripts/build_readme_showcase.py`. Use before-and-after images when explaining edits. Keep full coverage in `docs/gallery.md` and the packs; avoid repeated update announcements and long runs of identical preview tables.
