...
VideoWeb 版本另补充了[3条创作者配方](../prompts/16-videoweb-x-creators.md)和[图片到视频流程](videoweb-workflow.md)。下方沿用源库的10条路线；玩法7与9由 VideoWeb 补充了衔接提示词，尚未生成对应的新示例，原始生成记录与核对日期保留。
...
先用[P038](../prompts/07-stories-games.md#p038-zh)生成信使三视图，检查人物、衣服和配饰后保存。也可下载[信使示例](../assets/images/example-p038.png)作为练习输入。P037 和 P060 的默认主角是机器人；继续使用信使时，分别执行下面两段适配指令。

**第1步：上传确认的信使三视图，只复制本段生成故事。** 这一步借用 P037 的六格叙事结构，由参考图决定主角。

```text
以唯一上传的信使三视图作为角色身份参考，制作一张3:2横向六格无字故事板，三列两行。
各格只画同一位信使，保持参考图中的脸、发型、雨衣、挎包和靴子；三视图不作为分镜内容。
从左到右、从上到下依次表现：在阁楼发现裂成两半的纸月亮；拿起未缝合的两半；挑选橙线，此时纸月亮仍未缝合；用橙线缝合纸月亮；把修好的纸月亮挂起；坐在悬挂纸月亮的暖光下休息。
保持阁楼结构、道具和光线连贯，每格一个动作，象牙色格缝，温暖手绘质感。
恰好六格，每格最多一个完整人物，不复制三视图，不添加对白、编号、说明文字或其他角色。
```

检查六格的角色和道具状态，修正后保存确认板。新增信使路线未单独生图；不要把下方机器人示例当成它的结果。

**第2步：只上传刚确认的六格故事板，扩展右下角结尾。** 用这段替代默认 P060；需要其他镜头时，先同时替换格子位置与动作描述。

```text
以唯一上传的六格故事板为参考，只把右下角第6格扩展成一张16:9横向独立画面。
保留该格信使的脸、发型、衣服、挎包、靴子、坐姿和视线，保留缝合后的纸月亮、橙色缝线、阁楼结构及暖光。
只向左右自然延伸环境，保持该格机位与人物比例，不加入其他格的动作。
输出一张画面，不要格缝、边框、编号、文字、拼贴或额外人物。
```
...
先用[P031](../prompts/06-brand-ui.md#p031-zh)确认 `SOFT KILN` 字标，保存图片；也可下载[修正后的字标示例](../assets/images/example-p031-v2.png)练习。P035 的默认品牌是 `FIELD LETTER`，正文和约束都限定了该名称，不能原样用于当前品牌。

**上传一张确认字标图，只复制下面这段适配版。** 它借用 P035 的三盒陈列结构，改为陶艺品牌的杯、碗、花瓶包装。参考图只决定字标和配色，盒型由文字决定。此适配版尚未单独生图。

```text
以唯一上传的SOFT KILN确认字标图作为品牌参考，制作一张3:2横向陶艺包装概念图。
三只相同尺寸的长方体纸盒正面朝向镜头，等比例并排放在中性展台上；不展示盒内实物。
每盒正面都使用参考图中的SOFT KILN字标，保持字形、字距及相对比例；另分别标注CUP、BOWL、VASE。
沿用参考图的暖象牙白、黑色和陶土色，统一留白、字标位置、标签基线及盒型。
参考图里的反白演示矩形仅用于理解配色，不把整张字标展示板当成盒面图案。
呈现自然纸纤维和柔和影棚阴影。恰好三个纸盒，只出现SOFT KILN、CUP、BOWL、VASE，不添加口号、重量、认证、条码或其他文字。
```

检查三处品牌拼写与参考字形；需要精确保留字标时，在设计软件中叠加确认稿，不把生成图视为可直接印刷的文件。使用自有品牌时，先把本段所有品牌名、配色和品类文字统一替换，再上传对应字标。
