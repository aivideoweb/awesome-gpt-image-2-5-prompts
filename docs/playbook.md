...
由 [flaq.ai](https://flaq.ai) 团队整理。这里把源库的103条提示词串成可操作的创作路线：准备素材、选配方、检查结果，再继续下一步。路线组合不重复计入配方数量，也不代表这些玩法是2.5独占功能。

第一次使用建议从“商品换色”或“海报换字”开始：目标小、结果易比较。模型选择和已核实升级说明见 [Image 2与2.5对比](images-2-vs-2-5.md)。

VideoWeb 版本另补充了[3条创作者配方](../prompts/16-videoweb-x-creators.md)和[图片到视频流程](videoweb-workflow.md)。下方保留源库的10条创作路线及原始核对日期。
