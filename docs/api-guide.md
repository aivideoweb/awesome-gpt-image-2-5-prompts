...
These are **documentation examples, not executed API tests**. They use OpenAI’s API directly. They do not claim compatibility with a third-party endpoint or demonstrate that VideoWeb exposes the same API model IDs.

For the browser workflow, use [VideoWeb GPT Image 2.5](https://videoweb.ai/model/gpt-image-2-5/) or the [free no-signup tool](https://videoweb.ai/free-gpt-image-2-5/). The direct OpenAI code below is optional for developers and does not describe a VideoWeb API.
